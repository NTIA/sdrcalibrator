...
class SDR(object):
    ...
    SDR_DEFAULT_PERFORM_CALIBRATION_AT_STARTUP = True
    ...
    def connect(self, connect_params):
        ...
        self.wait_for_completion()
        if not "Agilent Technologies,N9020A" in self.idn:
            raise SDR_Error(
                    0,
    ...
    def get_serial_number(self):
        start_index = self.idn.find(',',25)+1
    ...
    def take_iq_samples(self, n, n_skip, retries = 5):
        ...
        self.set_and_check_parameter(':SENS:WAV:SWE:TIME', "{:.3f} ms".format(time*1e3), vartype="float6", param_val=int(round(1e6*time)))
