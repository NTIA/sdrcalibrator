...
class SDR(object):
    ...
    def gain_to_ref_level(self, gain):
        return int(self.max_reference_level-gain)
    def ref_level_to_gain(self, ref_level):
        return int(self.max_reference_level-ref_level)

    # Handle the gain settings for the SDR
    def set_gain(self, gain):
        gain = int(gain)
        ref_level = self.gain_to_ref_level(gain)

        CONFIG_SetReferenceLevel(ref_level)
        msg = "Set Tektronix RSA reference level: {:.1f} dB"
        print(msg.format(CONFIG_GetReferenceLevel()))
    ...
    def get_gain(self):
        ref_level = CONFIG_GetReferenceLevel()
        return self.ref_level_to_gain(ref_level)
        ...
        while True:
            try:
                result_data = iqstream_tempfile(
                    ...
                    self.gain_to_ref_level(self.get_gain()),
